...
interface LayoutProps {
  ...
  currentView: 'dashboard' | 'data-entry';
  onViewChange: (view: 'dashboard' | 'data-entry') => void;
}
  ...
  return (
    <div className="min-h-screen flex bg-slate-50 relative overflow-hidden">
      ...
      >
        ...
        <div className="flex-1 px-4 overflow-y-auto scrollbar-thin space-y-8">
          ...
          <nav className="space-y-1">
            <h3 className="px-4 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3">System Integration</h3>
          ...
          <nav className="space-y-1">
            <h3 className="px-4 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3">Configuration</h3>
        ...
        <header className="h-20 bg-white/80 backdrop-blur-md border-b border-slate-200 flex items-center justify-between px-8 sticky top-0 z-30 no-print">
          <div className="flex items-center gap-6">
            ...
            <div className="flex flex-col">
              <h2 className="text-sm font-black text-slate-900 tracking-widest uppercase">
                {currentView === 'dashboard' ? 'Operations Command' : 'Data Entry Portal'}
              </h2>
              <div className="flex items-center gap-2 mt-1">
                <div className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse"></div>
                <span className="text-[9px] font-black text-slate-400 uppercase tracking-wider">
                  {currentView === 'dashboard' ? 'Real-time Analytics Engine' : 'Secure Data Input Pipeline'}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            <div className="hidden sm:flex items-center gap-2 px-4 py-2 bg-slate-50 rounded-xl border border-slate-100">
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none pt-0.5">Live Traffic</span>
              <span className="text-xs font-bold text-green-600 leading-none">Healthy</span>
            </div>
