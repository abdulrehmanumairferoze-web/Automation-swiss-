        ...
        <div className="max-h-[600px] overflow-auto scrollbar-thin">
            {Object.keys(currentMapping).map(team => {
                ...
                const shortfallRows = masterRows.filter(r => {
                    ...
                    // Filter by search term (Search metric or team)
                    const matchesSearch =
                        r.metric.toLowerCase().includes(searchTerm.toLowerCase()) ||
                        r.team.toLowerCase().includes(searchTerm.toLowerCase());

                    // FOR TERRITORY SALES: Show all rows that match search
                    // FOR OTHERS: Only show rows with a GAP (Shortfall)
                    if (currentDept === 'Territory Sales') return matchesSearch;
